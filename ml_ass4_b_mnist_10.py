...
if not os.path.exists(LOGS_DIR):
    os.mkdir(LOGS_DIR)
MODEL_LOG_DIR = os.path.join(LOGS_DIR, "mnist_shallow_10")
...
def build_model(num_features: int, num_classes: int) -> Sequential:
    ...
    model = Sequential()
    model.add(Dense(units=10, kernel_initializer=init_w, bias_initializer=init_b, input_shape=(num_features,),))
