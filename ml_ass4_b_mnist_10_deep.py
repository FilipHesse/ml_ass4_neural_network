...
MODEL_DIR = os.path.abspath("/home/filip/Coding/UdemyTF/models")
if not os.path.exists(MODEL_DIR):
    os.mkdir(MODEL_DIR)
MODEL_FILE_PATH = os.path.join(MODEL_DIR, "mnist_model.h5")
LOGS_DIR = os.path.abspath("/home/filip/Coding/UdemyTF/logs")
if not os.path.exists(LOGS_DIR):
    os.mkdir(LOGS_DIR)
MODEL_LOG_DIR = os.path.join(LOGS_DIR, "mnist_deep_500_200_100")
...
def build_model(num_features: int, num_classes: int) -> Sequential:
    ...
    model.add(Dense(units=200, kernel_initializer=init_w, bias_initializer=init_b, input_shape=(num_features,),))
    model.add(Activation("relu"))
